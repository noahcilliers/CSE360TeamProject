...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
    	...
    	// Check if the user is attempting to use their one time password 	
    	if(password.compareTo("") != 0 && password.compareTo(oneTimePassword) == 0) {
    		
    		// If the user logs in with their one time password, erase it from the database and continue logging in.
    		theDatabase.updateOneTimePassword(username, "");
    	}
    	// If they don't use their one time password, continue like normal
    	...
    	User user = new User(username, actualPassword, theDatabase.getCurrentFirstName(), 
    	...
    	System.out.println("created user " + user.getUserName() + " : " + user.getPassword());
